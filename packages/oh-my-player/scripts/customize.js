...
switch(process.argv[2]) {
  ...
  case 'start':
    rewireModule('react-scripts/scripts/start.js', loadCustomizer('../configs/custom.dev'));
    break;
  // The "build" script is run to produce a production bundle
  case 'build':
    rewireModule('react-scripts/scripts/build.js', loadCustomizer('../configs/custom.prod'));
    break;
  ...
  case 'test':
    ...
    let customizer = loadCustomizer('../configs/custom.test');
